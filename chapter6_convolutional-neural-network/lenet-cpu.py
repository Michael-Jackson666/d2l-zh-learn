...
if __name__ == '__main__':
    ...
    # --- 加载数据 ---
    # num_workers 参数说明：
    #   num_workers 控制 PyTorch DataLoader 读取数据时的子进程数量。
    #   - num_workers=0：数据加载在主进程中进行，最兼容，适合 Windows/macOS 或调试。
    #   - num_workers>0：数据加载用多个子进程并行，能加速数据读取，适合 Linux 服务器。
    #   - 在 macOS 上，部分 Python 版本/环境下多进程可能不稳定，建议用 num_workers=0。
    #   - d2l.load_data_fashion_mnist 的部分版本不支持 num_workers 参数，直接去掉即可。
    #   - 推荐做法：
    #       1. 先尝试 train_iter, test_iter = d2l.load_data_fashion_mnist(batch_size)
    #       2. 如果你用原生 DataLoader，可根据平台设置 num_workers。
    ...
    if str(device) == 'mps' and not torch.backends.mps.is_available():
        raise RuntimeError("检测到 Apple 芯片，但 PyTorch mps 不可用。请升级 PyTorch 或检查环境配置。")
