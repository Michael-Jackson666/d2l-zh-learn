...
class Seq2SeqMultiHeadAttentionDecoder(AttentionDecoder):
    """带有多头注意力的序列到序列解码器"""
    def __init__(self, vocab_size, embed_size, num_hiddens, num_layers,
                 num_heads, dropout=0, **kwargs):
        super(Seq2SeqMultiHeadAttentionDecoder, self).__init__(**kwargs)
...
# 2. 测试MultiHeadAttention类
print("--- 2. 测试MultiHeadAttention类 ---")
...
encoder = Seq2SeqEncoder(vocab_size=10, embed_size=8, num_hiddens=16,
                             num_layers=2)
encoder.eval()
decoder = Seq2SeqMultiHeadAttentionDecoder(vocab_size=10, embed_size=8, 
...
# 4. 训练模型
print("\n--- 4. 训练带有多头注意力的Seq2Seq模型 ---")
embed_size, num_hiddens, num_layers, dropout = 32, 32, 2, 0.1
batch_size, num_steps = 64, 10
lr, num_epochs, device = 0.005, 50, try_gpu()
...
print(f"\n--- 多头注意力模型训练和预测完成 ---")
print(f"使用了 {num_heads} 个注意力头进行并行计算")
print("多头注意力相比单头注意力能够捕获更丰富的特征表示")